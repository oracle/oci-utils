...
if [ -f /usr/local/git/etc/mkks-iso-version ] || \
  [ -f /etc/OSCC-Release ] ; then
   echo "
...
# Check OS release
rpm -qf %{os_id} --qf '%{VERSION}' | egrep "%{os_version}" > /dev/null 2>&1

if [ $? -ne 0 ] ; then
  echo "
 Installed OS is not supported by this version of uln-be-setup.
   "
  exit 1
fi
