...
def partition_growfs_X(partition_data, dry_run=True):
    ...
    """
    cmd = [find_exec_in_path('growpart'), partition_data.device, partition_data.partition_number]
    if dry_run:
        cmd.append('--dry-run')
        msg = 'dry run'
    else:
    ...
    try:
        ...
        _logger.debug('growpart %s: %s', msg, output)
        return output
    except Exception as e:
        _logger.debug('Failed to execute growfs %s on filesystem %s: %s', msg, partition_data.mountpoint, str(e))
        _logger.error('Failed to execute growfs %s on filesystem %s', msg, partition_data.mountpoint)
        return None
