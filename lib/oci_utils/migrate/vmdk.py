...
class VmdkHead(DeviceData):
    ...
    def __init__(self, filename, logger=None):
        ...
        migrate_utils.progmsg('Got image %s header' % filename)
    ...
    def show_header(self):
        ...
        for f in VmdkHead.header0_structure:
            print '  %30s :' % f[2], f[1] % self.vmdkhead_dict[f[2]]
        print '\n  %30s\n  %30s   %30s' % \
              ('VMDK file descriptor data', '-' * 30, '-' * 30)
        for k in sorted(self.vmdkdesc_dict):
            print '  %30s : %-30s' % (k, self.vmdkdesc_dict[k])
    ...
    def image_size(self):
        ...
        migrate_utils.progmsg('Image size: physical %10.2f GB, '
                              'logical %10.2f GB' %
    ...
    def image_data(self):
        ...
        Returns
        -------
            dict: The data about the image and its contents.
        """
        ...
        try:
            ...
            migrate_utils.progmsg('Mounted %s' % self.devicename)
            deviceinfo = self.get_image_data()
    ...
    def type_specific_prereq_test(self):
        ...
        if self.img_info['img_size']['logical'] > prereqs['MAX_IMG_SIZE_GB']:
            ...
            failmsg += '\nImage size %8.2f GB exceeds maximum ' \
                       'allowed %8.2f GB' % \
                       (prereqs['MAX_IMG_SIZE_GB'],
                        self.img_info['img_size']['logical'])
            thispass = False
        else:
            migrate_utils.progmsg('Image size %8.2f GB meets maximum allowed '
                                  'size of %8.2f GB' %
                                  (self.img_info['img_size']['logical'],
                                   prereqs['MAX_IMG_SIZE_GB']))
